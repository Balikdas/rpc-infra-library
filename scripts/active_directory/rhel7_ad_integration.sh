...
# This script will integrate a RHEL7 host with Rogers Active Directory. Note that 
...
echo 'Starting RHEL7 AD integration...'

# This procedure only for RHEL7
grep 'Red Hat Enterprise Linux Server release 7' /etc/redhat-release
if [[ $? -ne 0 ]]; then
  echo "This script is only for RHEL 7.x. Terminating"
  exit 1
fi
...
# Install packages
yum install -y realmd adcli sssd sssd-ad samba-common-tools krb5-workstation oddjob oddjob-mkhomedir authselect
if [[ $? -ne 0 ]]; then
