...
# This script will integrate a Ubuntu 24 host with Rogers Active Directory. Note that 
# internet access and DNS integration is a prerequisite. Uncomment one of the
...
echo 'Starting Ubuntu AD integration...'

# This procedure only for Ubuntu 24
grep PRETTY_NAME /etc/os-release | grep -q "Ubuntu 24"
if [[ $? -ne 0 ]]; then
  echo "This script is only for Ubuntu 24.x. Terminating"
  exit 1
fi
...
# Install packages
apt -y install realmd adcli sssd sssd-ad sssd-ad-common samba-libs sssd-krb5 sssd-krb5-common oddjob oddjob-mkhomedir 
if [[ $? -ne 0 ]]; then
  echo "Package installation failed. Check internet conenctivity status. Terminating"
  exit 1
fi
